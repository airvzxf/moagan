...
8 unique test names skipped via `--skip` in `cargo test`. All 8 are
shared between `Makefile::test-ci` and both `ci.yml` jobs
(`test-lib`, `test-tests`).

| Skipped test | Origin | Reason |
|---|---|---|
| `audit_e2e_deep_run_has_exact_external_coverage` | Pre-existing (PR #232) | Proxy timing race (see `src/audit/proxy.rs:797`) |
| `cli::diff::tests::diff_unknown_run_returns_invalid_state` | PR #234 | SQLite pool race in CI cold cache |
| `validators::rust_validator::tests::good_rust_passes_when_cargo_present` | PR #234 | Sandbox cargo offline in cold cache |
| `validators::rust_validator::tests::good_rust_runs_three_steps_in_order` | PR #234 | Same as above |
| `validators::rust_validator::tests::rust_validator_fails_when_test_fails` | PR #234 | Same as above |
| `validators::rust_validator::tests::rust_validator_marks_no_tests_as_skipped` | PR #234 | Same as above |
| `validators::rust_validator::tests::rust_validator_passes_when_test_passes` | PR #234 | Same as above |
| `llm::response_format_opt_out::tests::env_var_extends_opt_out` | PR #237 | Flaky, panics at `src/llm/response_format_opt_out.rs:100` |

Total: **8 tests skipped via `--skip` CLI flag**.

Skip list lives in three places (must stay in sync):

- `Makefile:66-74` — `test-ci` target
- `.github/workflows/ci.yml:117-125` — `test-lib` job
- `.github/workflows/ci.yml:138-146` — `test-tests` job
...
| 2 | `cargo test --skip` CLI flag | 8 tests | ✅ yes (cold cache) |
| 3 | `#[ignore]` Rust attribute | 2 tests | ❌ no (run via `--ignored`) |
...
**Total tests actively skipped on CI:** 8 (`--skip` flag).
**Total tests in conditional skip code paths:** 60 (card80 subgroups + API-key block).

## How to add a new skip

1. **If the test is flaky in CI cold cache:** add it to:
   - `Makefile:66-74` (the `test-ci` target)
   - `.github/workflows/ci.yml:117-125` (test-lib job)
   - `.github/workflows/ci.yml:138-146` (test-tests job)

   In that exact order to keep the diff minimal. Document why in the
   commit body.
...
3. **If the test depends on an external binary that's not always
   ...
   missing). Document the binary in the table above.
...
1. **For `--skip` flag:** run the test locally with `cargo test
   ...
   If it passes consistently, remove from all 3 skip lists.
