...
// method on each provider (e.g. `OpenAICompatProvider::url`).
...
impl OpenAICompatProvider {
    /// Compute the URL the provider POSTs to. Kept as a
...
data: [DONE]\n\n",
    ...
    /// v0.10 (post Phase 8): the legacy 16_384-token global clamp on
    /// the chat-completions wire is gone. The OpenCode relay on the
    /// chat-completions path no longer has a per-kind ceiling baked
    ...
    /// that the clamp chain no longer applies that ceiling
        ...
        rt.block_on(async {
            ...
            let p = OpenAICompatProvider::new(
                &ProviderConfig {
                    ...
                    // global 16_384-token cap on the chat-completions
                    // wire; with nothing in the chain the wire
            ...
            // chat-completions wire is gone. With no operator
            // override and no table entry, the wire body carries
            // the requested value verbatim — the only layers left
            // in the clamp chain are `u32::MAX` (operator cap,
    ...
    /// unchanged — the v0.10 schema removed the global 16_384-token
    /// cap on the chat-completions wire, so there is no implicit
    /// ceiling.
    #[test]
...
data: [DONE]\n\n",
            ...
            // v0.10: the v0.9 16_384-token global ceiling on the
            // chat-completions wire is gone. With no operator
            // override and no table entry, the SSE wire body carries
            // the requested value verbatim — same contract as the
            // non-streaming path so a `stream: true` flip cannot
