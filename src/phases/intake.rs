...
/// #927: build a `ProviderRegistry` whose default lookup resolves
/// to a `ScriptedLlmClient`. The SDK stub is wrapped in
/// [`LlmClientProvider`] (the `Arc<dyn LlmClient>` →
/// `Arc<dyn Provider>` bridge) and inserted via
/// [`ProviderRegistry::insert`], which auto-wraps the raw provider
/// in a `BreakeredProvider`. `RunContext::llm_client()` then
/// resolves through the `BreakeredClient` adapter so the tests
/// exercise the new SDK-trait surface end-to-end. None of the
/// intake tests invoke the LLM directly; the registry only needs
/// to be populated so `llm_client()` does not panic if a future
/// test does.
#[cfg(test)]
