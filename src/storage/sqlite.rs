//! SQLite meta-database. Indexes the filesystem layout; never the
//! source of truth.
//!
...
/// Like v007 the migration is idempotent so a
...
impl Db {
    ...
    /// SQLite mirror is a queryable index.
    ///
...
mod tests {
    ...
    fn compare_runs_preserves_input_order() {
        ...
        let c = RunId::new();
        for id in [a, b, c] {
            db.register_run(id, "fast", "completed", "0.9.1", None, None, None)
                .unwrap();
            seed_cross_call(
                &db,
                &format!("c-{id}"),
    ...
    fn aggregates_window_sums_counts_and_percentiles() {
        ...
        db.register_run(old_run, "fast", "completed", "0.9.1", None, None, None)
            .unwrap();
    ...
    fn aggregates_window_provider_filter_narrows_runs() {
        ...
        db.register_run(minimax_run, "fast", "completed", "0.9.1", None, None, None)
            .unwrap();
        db.register_run(opencode_run, "fast", "completed", "0.9.1", None, None, None)
            .unwrap();
