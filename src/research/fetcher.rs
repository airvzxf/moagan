...
/// Hard cap on URLs accepted by a single [`fetch_all`] call.
...
pub async fn fetch_all(urls: &[String]) -> Vec<std::result::Result<ResearchSnippet, FetchError>> {
    ...
    for url in urls {
        out.push(fetch_one(&client, &policy, url).await);
    }
    out
}

/// Internal per-URL fetch + redact. `pub(crate)` so the in-module
/// test suite can exercise the host-allowlist branch without a
/// network round-trip.
...
mod tests {
    ...
    async fn fetch_one_rejects_disallowed_host_without_network() {
        ...
        let err = fetch_one(&client, &policy, url)
            .await
    ...
    async fn fetch_one_rejects_hostless_url() {
        ...
        let result = fetch_one(&client, &policy, url).await;
