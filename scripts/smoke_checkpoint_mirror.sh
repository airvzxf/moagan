...
run_test "s6_schema_user_version_is_15_after_open" "$(cat <<'EOF'
set -e
TMP=$(mktemp -d)
...
test "$v" = "15" || { echo "user_version=$v"; exit 1; }
EOF
)"
...
run_test "s6_schema_run_migrations_lists_15_versions" "awk '/Run pending migrations/{f=1} f{print}' ${ROOT}/src/storage/sqlite.rs | grep -cE 'current < [0-9]+' | grep -qE '^15$'"
...
test "$v1" = "$v2" && test "$v1" = "15" || { echo "v1=$v1 v2=$v2"; exit 1; }
EOF
)"
