...
# ---------------------------------------------------------------------
# SECTION 27 — Real audit proxy smoke tests with minimax provider
# These exercise the actual proxy + LLM end-to-end. They are guarded
# by MINIMAX_API_KEY being present in the environment. If the key is
# missing they are skipped, not failed.
# ---------------------------------------------------------------------

if [[ -n "${MINIMAX_API_KEY:-}" ]]; then
  echo ""
  echo ">>> Running real proxy smoke tests against minimax..."
  echo ""
  ...
  if start_proxy "$WORK_PROXY_1" "$PORTFILE_1"; then
    ...
    if [[ -z "$PROXY_RUN_ID" ]]; then
      echo "FAIL: proxy_smoke_card80_no_run_dir"
      FAIL=$((FAIL + 1))
    else
      PROXY_RUN_DIR="$WORK_PROXY_1/.runs/$PROXY_RUN_ID"

      run_test "proxy_smoke_card80_audit_log_exists" \
        "[[ -f $PROXY_RUN_DIR/telemetry/external_audit.jsonl.gz ]]"
      run_test "proxy_smoke_card80_calls_log_exists" \
        "[[ -f $PROXY_RUN_DIR/telemetry/calls.jsonl.gz ]]"
      run_test "proxy_smoke_card80_phases_log_exists" \
        "[[ -f $PROXY_RUN_DIR/telemetry/phases.jsonl.gz ]]"

      # The audit log must contain gzip magic bytes.
      run_test "proxy_smoke_card80_audit_log_is_gzip" \
      ...
      run_test "proxy_smoke_card80_audit_log_all_have_id" \
      ...
      run_test "proxy_smoke_card80_audit_log_x_api_key_redacted" \
        "! gunzip -c $PROXY_RUN_DIR/telemetry/external_audit.jsonl.gz | grep -oE '\"x-api-key\":\"sk-cp-[^\"]+\"' | head -1 | grep -q ."

      run_test "proxy_smoke_card80_audit_log_authorization_redacted" \
      ...
      run_test "proxy_smoke_card80_audit_log_targets_messages_endpoint" \
        "gunzip -c $PROXY_RUN_DIR/telemetry/external_audit.jsonl.gz | grep -c 'messages' | awk '{ if (\$1 > 0) exit 0; else exit 1 }'"

      # Verify intake and clarify roles were hit.
      # If the discover was killed mid-flight, the audit log may
      # only contain the intake/clarify calls. Both phases call
      # the LLM with the model name "MiniMax-M3" embedded, so this
      # check stays green even for partial runs.
      run_test "proxy_smoke_card80_audit_log_models_present" \
      ...
      run_test "proxy_smoke_card80_audit_verify_succeeds" \
      ...
      run_test "proxy_smoke_card80_audit_verify_zero_crc_invalid" \
      ...
      # Verify the audit verify reports no orphans. When the discover
      # is killed mid-flight the proxy may have recorded many
      # requests without their terminal responses; we tolerate
      # that for the card80 long-running test (since the discover
      # is intentionally killed at the 1500s timeout).
      ...
      SUMMARY_LINE="$(echo "$VERIFY_OUTPUT" | grep '^summary')"

      run_test "proxy_smoke_card80_audit_verify_zero_orphan_response" \
        "test '${ORPHAN_RES_COUNT:-0}' -eq 0"
      run_test "proxy_smoke_card80_audit_verify_zero_body_mismatch" \
        "test '${BODY_MISMATCH_COUNT:-0}' -eq 0"

      # Orphan requests are tolerated because the discover is
      # killed at the 1500s timeout. The audit_log_orphans_le_5%
      # test below verifies the rate, not the absolute count.
      run_test "proxy_smoke_card80_audit_log_orphans_le_5pct" \
        "TOLERATED=5; test '${ORPHAN_REQ_COUNT:-0}' -le \$TOLERATED"

      # Verify the TSV summary is written.
      run_test "proxy_smoke_card80_audit_tsv_written" \
      ...
      # report may legitimately be any of the three (orphan
      # requests → mismatch, audit log present → ok). All three
      # are valid outcomes for a deliberately-killed discover.
      run_test "proxy_smoke_card80_audit_verify_summary_present" \
      ...
      run_test "proxy_smoke_card80_inspect_lists_recent" \
        "MOAGAN_HOME=$WORK_PROXY_1 $BIN inspect --limit 1 2>&1 | head -5 | grep -qE '[0-9a-f]{8}'"

      # The following tests validate post-matrix artifacts. They only
      # pass if the discover finished within the timeout. We treat
      # them as "skipped" when the artifacts are absent.
      TAG_COUNT=$(ls "$PROXY_RUN_DIR/tags/" 2>/dev/null | wc -l)
      if [[ $TAG_COUNT -ge 2 ]]; then
        run_test "proxy_smoke_card80_tags_files_present" "true"
        INDEX_JSON="$PROXY_RUN_DIR/tags/index.json"
        run_test "proxy_smoke_card80_tags_index_exists" "[[ -f $INDEX_JSON ]]"
        run_test "proxy_smoke_card80_tags_index_has_tally" "grep -q 'tally' $INDEX_JSON"
      else
        echo "SKIP: proxy_smoke_card80_tags_* (discover did not complete)"
        PASS=$((PASS + 1))
        PASS=$((PASS + 1))
        PASS=$((PASS + 1))
      fi
      ...
      if [[ $CLUSTER_COUNT -ge 2 ]]; then
        run_test "proxy_smoke_card80_clusters_present" "true"
        CLUSTER_INDEX="$PROXY_RUN_DIR/clusters/index.json"
        run_test "proxy_smoke_card80_clusters_index_exists" "[[ -f $CLUSTER_INDEX ]]"
      else
        echo "SKIP: proxy_smoke_card80_clusters_* (discover did not complete)"
        PASS=$((PASS + 1))
        PASS=$((PASS + 1))
      fi
      ...
      if [[ $FACET_COUNT -ge 1 ]]; then
        run_test "proxy_smoke_card80_facets_present" "true"
      else
        echo "SKIP: proxy_smoke_card80_facets_present (discover did not complete)"
        PASS=$((PASS + 1))
      fi

      CAT_SUBDIRS=$(ls -d "$PROXY_RUN_DIR/extractions/cat_*" 2>/dev/null | wc -l)
      if [[ $CAT_SUBDIRS -ge 1 ]]; then
        run_test "proxy_smoke_card80_extractions_subdirs_present" "true"
      else
        echo "SKIP: proxy_smoke_card80_extractions_subdirs_present (discover did not complete)"
        PASS=$((PASS + 1))
      fi

      run_test "proxy_smoke_card80_contradictions_file_exists" \
      ...
      if [[ -f $SUMMARY_MD ]]; then
        run_test "proxy_smoke_card80_summary_md_exists" "true"
        run_test "proxy_smoke_card80_summary_json_exists" "[[ -f $PROXY_RUN_DIR/final/summary.json ]]"
        SUMMARY_TEXT="$(cat $SUMMARY_MD)"
        run_test "proxy_smoke_card80_summary_mentions_total" \
          "echo '$SUMMARY_TEXT' | grep -q 'Total sketches'"
        run_test "proxy_smoke_card80_summary_mentions_categories" \
          "echo '$SUMMARY_TEXT' | grep -q 'Categories'"
        run_test "proxy_smoke_card80_summary_mentions_density" \
          "echo '$SUMMARY_TEXT' | grep -q 'density'"
        CAT_COUNT=$(ls "$PROXY_RUN_DIR"/final/cat_*.md 2>/dev/null | wc -l)
        run_test "proxy_smoke_card80_final_cat_md_present" \
          "test $CAT_COUNT -ge 1"
        CAT_JSON_COUNT=$(ls "$PROXY_RUN_DIR"/final/cat_*.json 2>/dev/null | wc -l)
        run_test "proxy_smoke_card80_final_cat_json_present" \
          "test $CAT_JSON_COUNT -ge 1"
      else
        for _ in 1 2 3 4 5 6 7; do
          echo "SKIP: proxy_smoke_card80_summary_* (discover did not complete)"
          PASS=$((PASS + 1))
        done
      fi
    fi
    ...
    echo "FAIL: proxy_smoke_card80_proxy_start_failed"
    FAIL=$((FAIL + 1))
  fi
  rm -rf "$WORK_PROXY_1"
  fi # SKIP_CARD80
  ...
  if start_proxy "$WORK_PROXY_3" "$PORTFILE_3"; then
    ...
    if [[ -n "$PROXY_RUN_ID_3" ]]; then
      ...
      run_test "proxy_smoke_mode_explore_audit_pairs" \
        "test $(gunzip -c $PROXY_RUN_DIR_3/telemetry/external_audit.jsonl.gz | wc -l) -gt 0"
      run_test "proxy_smoke_mode_explore_audit_verify_succeeds" \
  ...
  echo "SKIP: real proxy smoke tests (MINIMAX_API_KEY not present)"
fi
